...
export async function getQueryRunsResponseBrowser(clientId, limit = 120) {
    const withBodies = `id, tracked_query_id, query_text, provider, model, status, parse_status, parse_confidence, target_found, target_position, created_at, response_text, raw_response_full, parsed_response, ${QUERY_RUN_TRACKED_QUERY_SELECT}`;
    ...
    if (error) {
        ...
        if (missing === 'raw_response_full' || missing === 'response_text') {
            const fallback = `id, tracked_query_id, query_text, provider, model, status, parse_status, parse_confidence, target_found, target_position, created_at, parsed_response, ${QUERY_RUN_TRACKED_QUERY_SELECT}`;
