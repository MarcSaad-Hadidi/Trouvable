...
function isPrivateOrLocalIp(hostname) {
    ...
    // Bracketless IPv6 patterns commonly representing local/link-local/ULA
...
function isDisallowedHostname(hostname) {
    ...
    if (isPrivateOrLocalIp(lower)) return true;
    return false;
}
...
function toSafeHttpUrl(raw) {
    try {
        ...
        if (isDisallowedHostname(parsed.hostname)) return null;
        return parsed;
    } catch {
        return null;
    }
...
async function resolveLlmsTxtBodies(siteUrl, crawlerAccessData) {
    ...
    try {
        const safeBaseUrl = toSafeHttpUrl(siteUrl);
        if (!safeBaseUrl) {
            return { llmsTxtBody: null, llmsFullBody: null };
        }

        const llmsTxtUrl = new URL('/llms.txt', safeBaseUrl).toString();
        const llmsFullUrl = new URL('/llms-full.txt', safeBaseUrl).toString();

        const [llmsTxtRes, llmsFullRes] = await Promise.all([
            fetch(llmsTxtUrl, { headers: { 'User-Agent': 'TrouvableAuditBot/3.0', Accept: 'text/plain,*/*' }, redirect: 'follow' }).catch(() => null),
            fetch(llmsFullUrl, { headers: { 'User-Agent': 'TrouvableAuditBot/3.0', Accept: 'text/plain,*/*' }, redirect: 'follow' }).catch(() => null),
        ]);
