...
import { getSitemapDiscoveryConfig } from '../audit-config.js';
...
async function expandSitemap(sitemapUrl, visited, depth = 0) {
    const config = getSitemapDiscoveryConfig();
    if (depth > 2) return [];
    if (visited.size >= config.maxSitemapDocs) return [];
    ...
    for (const child of sitemapUrls) {
        if (visited.size >= config.maxSitemapDocs) break;
        const childPages = await expandSitemap(child, visited, depth + 1);
        discovered.push(...childPages);
        if (discovered.length >= config.maxUrlsFromSitemap) break;
    }
...
export async function discoverSitemapUrls(startUrl) {
    ...
    for (const raw of aggregated) {
        ...
        if (normalized.length >= config.maxUrlsFromSitemap) break;
    }
