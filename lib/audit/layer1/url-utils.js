...
const MAX_NOISY_PATH_QUERY_PARAMS = 3;
...
export function normalizeUrl(input, baseHref = null) {
    ...
    for (const [key, value] of parsed.searchParams.entries()) {
        ...
        if (!normalizedKey || TRACKING_PARAM_RE.test(normalizedKey)) continue;
        const dedupeKey = `${normalizedKey.toLowerCase()}=${value}`;
        if (seenParams.has(dedupeKey)) continue;
        seenParams.add(dedupeKey);
        retainedParams.push([normalizedKey, value]);
    }
