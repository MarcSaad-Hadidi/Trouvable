...
import { isSitemapFirstEnabled, getCrawlerRuntimeConfig } from './audit-config.js';
...
function shouldAttemptRenderedPage({
    ...
    if (renderAttempts >= renderAttemptLimit) return false;
...
export async function runSiteAudit(startUrl) {
    ...
    function recordSkipped(rawUrl, reason) {
        skippedReasonCounts[reason] = (skippedReasonCounts[reason] || 0) + 1;
        if (skippedUrls.length < 200) {
            skippedUrls.push({ url: String(rawUrl || '').slice(0, 2048), reason });
        }
    }
            ...
            if (sitemapDiscovery.discovered_urls.length > 0) {
                ...
                for (const url of sitemapDiscovery.discovered_urls) {
                    ...
                    if (!isLikelyHtmlUrl(url)) {
                        recordSkipped(url, 'non_html_url');
                        continue;
                    }
                    if (!sameOrigin(url, baseUrlObj.href)) {
                        recordSkipped(url, 'external_host');
                        continue;
                    }
                    if (queuedUrls.has(url) || visitedPages.has(url)) {
                        recordSkipped(url, 'duplicate_url');
                        continue;
                    }
            ...
            try {
                ...
                if (fetchFailed) {
                    ...
                    shouldRender = shouldAttemptRenderedPage({
                        ...
                        renderAttempts,
                        renderAttemptLimit: safety.renderAttemptLimit,
                        pageIndex,
                ...
                if (visitedPages.size <= safety.linkExtractionDepth) {
                    $('a[href]').each((_, link) => {
                        ...
                        if (!normalizedHref) {
                            recordSkipped(href, 'invalid_or_noisy_url');
                            return;
                        }
                        if (!sameOrigin(normalizedHref, baseUrlObj.href)) {
                            recordSkipped(normalizedHref, 'external_host');
                            return;
                        }
                        if (!isLikelyHtmlUrl(normalizedHref)) {
                            recordSkipped(normalizedHref, 'non_html_url');
                            return;
                        }
                        if (visitedPages.has(normalizedHref) || queuedUrls.has(normalizedHref)) {
                            recordSkipped(normalizedHref, 'duplicate_url');
                            return;
                        }
    ...
    layer1Block.crawl_metadata.pages_failed = results.scanned_pages.filter((page) => !page.success).length;
    layer1Block.crawl_metadata.skipped_urls = skippedUrls;
    layer1Block.crawl_metadata.skipped_reasons = skippedReasonCounts;
    layer1Block.crawl_metadata.completed_at = new Date().toISOString();
    layer1Block.crawl_metadata.stopped_reason = stoppedReason;
    layer1Block.crawl_metadata.limit_reason = stoppedReason === 'frontier_exhausted' ? null : stoppedReason;
    layer1Block.crawl_metadata.queued_remaining = pagesToVisit.size;
