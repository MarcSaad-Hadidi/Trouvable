    ...
    return (
        <section className={cn(COMMAND_SURFACE_SOFT, 'p-5 sm:p-6')}>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">Récence</div>
                    <div className="mt-3 text-[22px] font-semibold text-slate-950">{title}</div>
                    {description ? <p className="mt-2 text-[13px] leading-relaxed text-slate-600">{description}</p> : null}
                </div>
            </div>

            {!items.length ? (
                <div className="mt-6 rounded-[22px] border border-dashed border-slate-300 bg-white p-6 text-center">
                    <div className="text-[15px] font-semibold text-slate-900">{emptyTitle}</div>
                    <p className="mx-auto mt-2 max-w-xl text-[13px] leading-relaxed text-slate-500">{emptyDescription}</p>
                </div>
            ) : (
                                ...
                                <div className="rounded-[20px] border border-slate-200 bg-white p-4">
                                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                        <div className="min-w-0">
                                            ...
                                            <div className="mt-1 text-[13px] leading-relaxed text-slate-600">{item.description}</div>
                                        </div>
                                    ...
                                    {item.href ? (
                                        <div className="mt-4">
                                            <Link href={item.href} className="inline-flex items-center gap-1 text-[12px] font-semibold text-slate-700 transition-colors hover:text-white">
                                                Ouvrir
